...
function HiddenMarkovModels.test_equal_hmms(
    hmm1::AbstractHMM, hmm2::AbstractHMM; control_seq=[nothing], atol::Real=0.1
)
    check_equal_hmms(hmm1, hmm2; control_seq, atol, test=true)
    return nothing
end
...
function HiddenMarkovModels.test_coherent_algorithms(
    ...
    if !isnothing(hmm_guess)
        ...
        test_equal_hmms(hmm, hmm_est; control_seq=control_seq[1:2], atol)
    end
