...
function benchmarkables_hmmlearn(; algos, N, D, T, K)
    ...
    if "logdensity" in algos
        ...
        ) setup = (model_score = rand_model_hmmlearn(; N=$N, D=$D).score)
    end
    if "viterbi" in algos
        benchs["viterbi"] = @benchmarkable pycall(
            model_predict, $obs_mat_concat_py, $obs_mat_len_py
        ) setup = (model_predict = rand_model_hmmlearn(; N=$N, D=$D).predict)
    end
    ...
    if "baum_welch" in algos
        ...
        ) setup = (model_fit = rand_model_hmmlearn(; N=$N, D=$D).fit)
    end
    return benchs
end
